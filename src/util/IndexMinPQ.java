...
public class IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    ...
    private int[] pq;        // binary heap using 1-based indexing   通过索引查到在堆中的实际存储位置
    private int[] qp;        // inverse of pq - qp[pq[i]] = pq[qp[i]] = i  根据索引找到在堆中的位置  
    private Key[] keys;      // keys[i] = priority of i   通过索引得到key
